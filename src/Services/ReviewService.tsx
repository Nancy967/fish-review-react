...
export const reviewPostAPI = async (
  ...
  try {
    const data = await axios.post<ReviewPost>(api + `${scientificName}`, {
      title: title,
      content: content,
    });
    return data;
...
export const reviewGetAPI = async (scientificName: string) => {
  try {
    // const data = await axios.get<ReviewGet[]>(api + `?Symbol=${scientificName}`);
    const data = {
      data: [
        {
          title: "活潑有趣的水族箱明星",
