...
type Props = {
  ...
  handleReview: (e: ReviewFormInputs) => void;
};
...
const FishReviewForm = ({ scientificName, handleReview }: Props) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ReviewFormInputs>({ resolver: yupResolver(validation) });
  return (
    <form className="mt-4 ml-4" onSubmit={handleSubmit(handleReview)}>
      <input
        type="text"
        id="title"
      ...
      <button
        type="submit"
        className="inline-flex items-center py-2.5 px-4 text-xs font-medium text-center text-white bg-neonOrange rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800"
      >
        Post review
      </button>
    </form>
