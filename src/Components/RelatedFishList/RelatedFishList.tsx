...
import { RelatedFishData } from "../../fish";
import { fetchRelatedFishes } from "../../api";
...
const RelatedFishList = ({ scientificName }: Props) => {
  const [fishData, setFishData] = useState<RelatedFishData>();
  useEffect(() => {
    const getRelatedFishes = async () => {
      const value = await fetchRelatedFishes(scientificName);
